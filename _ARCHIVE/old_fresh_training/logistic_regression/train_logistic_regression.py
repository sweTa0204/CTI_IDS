#!/usr/bin/env python3
"""
Logistic Regression Model Training - Fresh Execution
====================================================
...
def train_model(X_train, y_train):
    """Train Logistic Regression with proven hyperparameters"""
...
def generate_performance_figure(metrics, cm, y_test, y_pred_proba, feature_names, model):
    ...
    ax2.plot(fpr, tpr, 'red', linewidth=2, label=f'ROC (AUC = {metrics["roc_auc"]:.4f})')
    ...
    # 3. Feature Coefficients (Logistic Regression has coefficients)
    ...
    ax3.set_title('Feature Coefficients', fontsize=12, fontweight='bold', pad=10)
    ...
    plt.savefig('logistic_regression_performance.png', bbox_inches='tight', facecolor='white')
    plt.close()

    print("\nSaved: logistic_regression_performance.png")
...
def save_results(model, metrics, training_time, feature_names):
    ...
    coefficients = {feature: float(coef) for feature, coef in zip(feature_names, model.coef_[0])}
...
def main():
    """Main execution"""
    print("="*60)
    print("LOGISTIC REGRESSION MODEL TRAINING - FRESH EXECUTION")
    print("="*60)
    ...
    print("  - logistic_regression_performance.png (performance figure)")
