#!/usr/bin/env python3
"""
XGBoost Model Training - Fresh Execution
=========================================
...
def prepare_data(df):
    ...
    # Split with same random state for reproducibility
...
def train_model(X_train, y_train):
    """Train XGBoost with the proven hyperparameters"""
    print("\nTraining XGBoost model...")

    # Use the same hyperparameters that worked before
    model = XGBClassifier(
...
def generate_performance_figure(metrics, cm, y_test, y_pred_proba, feature_names, model):
    ...
    ax2.plot(fpr, tpr, 'b-', linewidth=2, label=f'ROC (AUC = {metrics["roc_auc"]:.4f})')
    ...
    bars = ax3.barh(importance_df['Feature'], importance_df['Importance'], color='steelblue')
    ...
    # Save figure
    plt.savefig('xgboost_performance.png', bbox_inches='tight', facecolor='white')
    plt.close()

    print("\nSaved: xgboost_performance.png")
...
def main():
    """Main execution"""
    print("="*60)
    print("XGBOOST MODEL TRAINING - FRESH EXECUTION")
    print("="*60)
    ...
    print("  - xgboost_performance.png (performance figure)")
