#!/usr/bin/env python3
"""
SVM Model Training - Fresh Execution
====================================
...
def train_model(X_train, y_train):
    """Train SVM with proven hyperparameters"""
    print("\nTraining SVM model...")
...
def generate_performance_figure(metrics, cm, y_test, y_pred_proba, feature_names):
    ...
    ax2.plot(fpr, tpr, 'purple', linewidth=2, label=f'ROC (AUC = {metrics["roc_auc"]:.4f})')
    ...
    # 3. Model Info (SVM doesn't have feature importance)
    ...
    Support Vectors: {len(feature_names)} features
    """
    ax3.text(0.5, 0.5, info_text, transform=ax3.transAxes, fontsize=12,
             verticalalignment='center', horizontalalignment='center',
             bbox=dict(boxstyle='round', facecolor='lavender', alpha=0.8))
    ax3.set_xlim([0, 1])
    ax3.set_ylim([0, 1])
    ax3.axis('off')
    ax3.set_title('Model Configuration', fontsize=12, fontweight='bold', pad=10)
    ...
    plt.savefig('svm_performance.png', bbox_inches='tight', facecolor='white')
    plt.close()

    print("\nSaved: svm_performance.png")
...
def main():
    """Main execution"""
    print("="*60)
    print("SVM MODEL TRAINING - FRESH EXECUTION")
    print("="*60)
    ...
    generate_performance_figure(metrics, cm, y_test, y_pred_proba, feature_names)
    ...
    print("  - svm_performance.png (performance figure)")
