#!/usr/bin/env python3
"""
Random Forest LIME Comprehensive Analysis for DoS Detection
Complete local explanation analysis using LIME
Comparison with SHAP explanations and XGBoost LIME results
...
class RandomForestLIMEAnalyzer:
    """
    Comprehensive LIME analysis for Random Forest DoS detection model
    """
    ...
    def load_model_and_data(self):
        """Load Random Forest model and prepare data"""
    ...
    def analyze_sample_explanations(self, num_samples=15):
        ...
        for i, idx in enumerate(selected_indices, 1):
            ...
            try:
                ...
                fig.suptitle(f'Random Forest LIME - Sample {i} ({actual_label})', fontsize=14, fontweight='bold')
                plt.tight_layout()
    ...
    def create_feature_importance_analysis(self, feature_importance):
        ...
        plt.title('Random Forest + LIME - Global Feature Importance', fontsize=16, fontweight='bold')
        ...
        plt.title('Random Forest + LIME - Feature Importance Distribution', fontsize=16, fontweight='bold')
        plt.axis('equal')
        plt.tight_layout()
    ...
    def compare_with_shap_and_xgboost_lime(self, rf_lime_importance):
        """Compare Random Forest LIME with SHAP and XGBoost LIME results"""
        print("\n🔍 COMPARATIVE ANALYSIS: RF LIME vs SHAP vs XGBoost LIME")
        print("=" * 60)
        
        comparison_results = {}
        
        # 1. Compare with Random Forest SHAP
        try:
            ...
            shap_total = sum(rf_shap_importance.values())
            rf_shap_normalized = {k: v/shap_total for k, v in rf_shap_importance.items()}
            
            # Calculate correlation
            lime_values = [rf_lime_importance.get(f, 0.0) for f in self.feature_names]
            shap_values = [rf_shap_normalized.get(f, 0.0) for f in self.feature_names]
            rf_lime_shap_correlation = np.corrcoef(lime_values, shap_values)[0, 1]
            
            comparison_results['rf_lime_vs_rf_shap'] = {
                'correlation': rf_lime_shap_correlation,
                'lime_top_feature': max(rf_lime_importance.items(), key=lambda x: x[1]),
                'shap_top_feature': max(rf_shap_normalized.items(), key=lambda x: x[1])
            }
            
            print(f"🌳 Random Forest LIME vs SHAP Correlation: {rf_lime_shap_correlation:.3f}")
            
        except Exception as e:
            print(f"❌ Could not compare with RF SHAP: {e}")
        
        # 2. Compare with XGBoost LIME
        try:
            ...
            with open(xgb_lime_path, 'r') as f:
                xgb_lime_importance = json.load(f)
            
            # Calculate correlation
            rf_lime_values = [rf_lime_importance.get(f, 0.0) for f in self.feature_names]
            xgb_lime_values = [xgb_lime_importance.get(f, 0.0) for f in self.feature_names]
            rf_xgb_lime_correlation = np.corrcoef(rf_lime_values, xgb_lime_values)[0, 1]
            
            comparison_results['rf_lime_vs_xgb_lime'] = {
                'correlation': rf_xgb_lime_correlation,
                'rf_lime_top_feature': max(rf_lime_importance.items(), key=lambda x: x[1]),
                'xgb_lime_top_feature': max(xgb_lime_importance.items(), key=lambda x: x[1])
            }
            
            print(f"🏆 Random Forest LIME vs XGBoost LIME Correlation: {rf_xgb_lime_correlation:.3f}")
            ...
            plt.subplot(1, 3, 1)
            features = list(rf_lime_importance.keys())[:8]
            rf_lime_vals = [rf_lime_importance[f] for f in features]
            rf_shap_vals = [rf_shap_normalized.get(f, 0.0) for f in features]
            
            x = np.arange(len(features))
            width = 0.35
            
            plt.bar(x - width/2, rf_lime_vals, width, label='RF LIME', color='forestgreen', alpha=0.8)
            plt.bar(x + width/2, rf_shap_vals, width, label='RF SHAP', color='steelblue', alpha=0.8)
            
            plt.xlabel('Features')
            plt.ylabel('Importance')
            plt.title(f'RF LIME vs RF SHAP\n(r={rf_lime_shap_correlation:.3f})')
            plt.xticks(x, features, rotation=45)
            plt.legend()
            plt.grid(True, alpha=0.3)
            
            # RF LIME vs XGBoost LIME
            plt.subplot(1, 3, 2)
            xgb_lime_vals = [xgb_lime_importance.get(f, 0.0) for f in features]
            
            plt.bar(x - width/2, rf_lime_vals, width, label='RF LIME', color='forestgreen', alpha=0.8)
            plt.bar(x + width/2, xgb_lime_vals, width, label='XGBoost LIME', color='darkorange', alpha=0.8)
            
            plt.xlabel('Features')
            plt.ylabel('Importance')
            plt.title(f'RF LIME vs XGBoost LIME\n(r={rf_xgb_lime_correlation:.3f})')
            plt.xticks(x, features, rotation=45)
            plt.legend()
            plt.grid(True, alpha=0.3)
            ...
            plt.subplot(1, 3, 3)
            plt.scatter(rf_lime_values, xgb_lime_values, alpha=0.7, s=100, color='purple')
            
            # Add feature labels
            for i, feature in enumerate(self.feature_names):
                plt.annotate(feature, (rf_lime_values[i], xgb_lime_values[i]), 
                           xytext=(5, 5), textcoords='offset points', fontsize=9)
            
            # Add correlation line
            max_val = max(max(rf_lime_values), max(xgb_lime_values))
            plt.plot([0, max_val], [0, max_val], 'r--', alpha=0.5, label='Perfect Correlation')
            
            plt.xlabel('Random Forest LIME')
            plt.ylabel('XGBoost LIME')
            plt.title(f'LIME Cross-Model Correlation\n(r={rf_xgb_lime_correlation:.3f})')
            plt.legend()
            plt.grid(True, alpha=0.3)
            
            plt.tight_layout()
            plt.savefig(self.viz_dir / 'comparative_plots' / 'comprehensive_lime_comparison.png', 
                       dpi=300, bbox_inches='tight')
            plt.close()
            
        except Exception as e:
            print(f"❌ Could not compare with XGBoost LIME: {e}")
        
        # Save comparison results
        with open(self.results_dir / 'comprehensive_comparison.json', 'w') as f:
    ...
    def generate_comprehensive_report(self, explanations_data, importance_df, prediction_analysis, comparison_results):
        """Generate comprehensive Random Forest LIME analysis report"""
        print("\n📋 GENERATING COMPREHENSIVE RF LIME REPORT")
        print("=" * 50)
        
        report = f"""# RANDOM FOREST LIME COMPREHENSIVE ANALYSIS REPORT
...
"""
        ...
        if comparison_results:
            report += """## COMPREHENSIVE COMPARATIVE ANALYSIS

### Cross-Method Comparison:
"""
            
            if 'rf_lime_vs_rf_shap' in comparison_results:
                rf_shap_corr = comparison_results['rf_lime_vs_rf_shap']['correlation']
                report += f"- **RF LIME vs RF SHAP Correlation**: {rf_shap_corr:.3f}\n"
            
            if 'rf_lime_vs_xgb_lime' in comparison_results:
                xgb_lime_corr = comparison_results['rf_lime_vs_xgb_lime']['correlation']
                report += f"- **RF LIME vs XGBoost LIME Correlation**: {xgb_lime_corr:.3f}\n"
...
"""
        ...
        # Save report
        with open(self.doc_dir / 'randomforest_lime_report.md', 'w') as f:
            f.write(report)
        
        print("✅ Comprehensive Random Forest LIME report generated")
        print("📚 Report saved to documentation/randomforest_lime_report.md")
    
    def run_complete_analysis(self):
        """Execute complete Random Forest LIME analysis pipeline"""
        print("🔍 RANDOM FOREST LIME COMPREHENSIVE ANALYSIS")
        print("=" * 60)
        print("Runner-up Model (95.29%) + LIME Explanations")
        print("Complete Local and Cross-Model Comparative Analysis")
        print("=" * 60)
        ...
        try:
            ...
            comparison_results = self.compare_with_shap_and_xgboost_lime(feature_importance)
            
            # Generate comprehensive report
            self.generate_comprehensive_report(explanations_data, importance_df, prediction_analysis, comparison_results)
            
            print("\n🎉 RANDOM FOREST LIME ANALYSIS COMPLETED!")
            print("=" * 60)
            print("💡 Local explanations: DONE")
            print("📊 Feature importance: DONE")
            print("🎯 Prediction analysis: DONE")
            print("🔍 Cross-model comparison: DONE")
            print("📚 Comprehensive report: DONE")
            print("💾 All results saved to results/ directory")
            print("🎨 All visualizations saved to visualizations/ directory")
            
            print("\n🏆 RUNNER-UP MODEL LIME EXPLANATION COMPLETE!")
            print("✅ Random Forest (95.29%) is now explainable with LIME")
            print("💡 Local interpretable explanations available")
            print("🚀 Ready for final comprehensive XAI framework analysis")
            
        except Exception as e:
            print(f"❌ Error in Random Forest LIME analysis: {e}")
            import traceback
            traceback.print_exc()

if __name__ == "__main__":
    analyzer = RandomForestLIMEAnalyzer()
